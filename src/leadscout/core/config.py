...
class Settings(BaseSettings):
    ...
    openai_api_key: Optional[SecretStr] = Field(
        None, 
        env="OPENAI_API_KEY",
        description="OpenAI API key for name classification"
    )
    ...
    cache_dir: Path = Field(
        default=Path("./cache"),
        env="LEADSCOUT_CACHE_DIR",
        description="Directory for cache storage"
    )
    cache_ttl_days: int = Field(
        default=30,
        env="LEADSCOUT_CACHE_TTL_DAYS",
    ...
    confidence_threshold: float = Field(
        default=0.8,
        env="LEADSCOUT_CONFIDENCE_THRESHOLD",
        ge=0.0,
        le=1.0,
    ...
    enable_telemetry: bool = Field(
        default=True,
        env="LEADSCOUT_ENABLE_TELEMETRY",
    ...
    def validate_configuration(self) -> Dict[str, Any]:
        ...
        if not self.has_llm_keys():
            issues.append("No LLM API keys configured - name classification will not work")
        
        # Check cache directory
        if not self.cache_dir.exists():
            warnings.append(f"Cache directory does not exist: {self.cache_dir}")
        elif not self.cache_dir.is_dir():
            issues.append(f"Cache path is not a directory: {self.cache_dir}")
        
        # Check configuration values
        if self.max_concurrent > 20:
            warnings.append(f"High concurrent limit ({self.max_concurrent}) may hit API rate limits")
        ...
        for field_name, field in self.fields.items():
            ...
            if isinstance(value, SecretStr):
                if include_secrets:
                    ...
                    data[field_name] = "[HIDDEN]" if value else None
            # Handle Path fields
