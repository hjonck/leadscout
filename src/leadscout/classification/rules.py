...
from .dictionaries import NameDictionaries, EthnicityType, NameEntry, get_dictionaries
from .models import (
    Classification, ClassificationMethod, RuleClassificationDetails,
    MultiWordNameAnalysis, ValidationResult, AlternativeClassification
)
from .exceptions import (
    RuleClassificationError, NameValidationError, raise_invalid_name,
...
class RuleBasedClassifier:
    ...
    def __init__(self, dictionaries: Optional[NameDictionaries] = None):
        ...
        logger.info("RuleBasedClassifier initialized with comprehensive SA dictionaries")
    ...
    def validate_name(self, name: str) -> ValidationResult:
        ...
        name_parts = [part.strip() for part in re.split(r"[\s\-]+", normalized) if part.strip()]
        ...
        for part in name_parts:
            if len(part) == 1:
                normalized_parts.append(part.upper())  # Single letter -> uppercase
    ...
    def classify_name(self, name: str) -> Optional[Classification]:
        ...
        try:
            ...
            if validation.is_multi_word:
                ...
                return self._classify_single_name(validation.normalized_name, start_time)
                
        except Exception as e:
            logger.error(f"Rule classification failed for '{name}': {e}")
            raise RuleClassificationError(
                f"Rule classification failed for '{name}': {str(e)}",
                name=name
            )
    
    def _classify_single_name(self, name: str, start_time: float) -> Optional[Classification]:
    ...
    def _classify_multi_word_name(self, validation: ValidationResult) -> Classification:
        ...
        analysis = MultiWordNameAnalysis(
            ...
            conflicting_ethnicities=len(set(c.ethnicity for c in individual_classifications)) > 1,
            european_elements=[c.name for c in individual_classifications if c.ethnicity == EthnicityType.WHITE],
            non_european_elements=[c.name for c in individual_classifications if c.ethnicity != EthnicityType.WHITE],
            reasoning=self._explain_priority_logic(individual_classifications, priority_classification)
        )
        
        # Update the classification with multi-word details
        priority_classification.rule_details.special_heuristic_applied = "multi_word_priority"
        
        return priority_classification
    
    def _apply_priority_logic(self, classifications: List[Classification]) -> Classification:
        """Apply priority logic: least European element wins."""
    ...
    def _explain_priority_logic(self, classifications: List[Classification], chosen: Classification) -> str:
        ...
        return (f"Multi-ethnic name with {', '.join(f'{e.value}({c})' for e, c in ethnic_counts.items())}. "
