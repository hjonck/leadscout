...
from .models import Classification, EthnicityType, ClassificationMethod
...
class LLMLearningDatabase:
        ...
        with self._db_lock:
            try:
                with sqlite3.connect(self.db_path, timeout=30.0) as conn:
                    ...
                    logger.info("LLM classification stored and learned",
                ...
                logger.error("Failed to store LLM classification",
                            extra={"failed_name": record.name,
                                  "error": str(e)})
                return False
    
    def _extract_and_store_patterns(self, conn: sqlite3.Connection, record: LLMClassificationRecord):
    ...
    def _store_structural_patterns(self, conn: sqlite3.Connection, record: LLMClassificationRecord):
        ...
        for prefix_key in ['prefix_2', 'prefix_3']:
            if prefix_key in features:
                ...
                if len(prefix_value) >= 2:
                    pattern_id = f"structural_{prefix_key}_{prefix_value}_{record.ethnicity}"
                    ...
                    conn.execute('''
                        ...
                        VALUES (?, ?, ?, ?, ?, 1)
                    ''', (pattern_id, f'structural_{prefix_key}', prefix_value, record.ethnicity, record.confidence))
    ...
    def _store_phonetic_family(self, conn: sqlite3.Connection, record: LLMClassificationRecord):
        ...
        for code_type, code_value in phonetic_codes.items():
            ...
            ''', (phonetic_codes.get('soundex', ''), record.ethnicity)).fetchone()
            ...
            if existing:
                ...
                new_confidence = (old_confidence * member_count + record.confidence) / (member_count + 1)
                ...
                ''', (
                    ...
                    phonetic_codes.get('nysiis', ''),  # Use nysiis instead of double_metaphone_primary
                    phonetic_codes.get('match_rating_codex', ''),  # Use match_rating instead of double_metaphone_secondary
                    record.ethnicity,
                    record.confidence,
                    json.dumps([record.name])
                ))
    
    def _store_linguistic_patterns(self, conn: sqlite3.Connection, record: LLMClassificationRecord):
    ...
    def find_learned_classification(self, name: str) -> Optional[Classification]:
        ...
        with self._db_lock:
            try:
                ...
                logger.error("Error in learned classification lookup",
                            extra={"lookup_name": name,
                                  "error": str(e)})
                return None
            
            finally:
                processing_time = (time.time() - start_time) * 1000
                logger.debug("Learned classification lookup completed",
                            extra={"lookup_name": name,
                                  "processing_time_ms": processing_time})
    
    def _check_classification_cache(self, conn: sqlite3.Connection, name: str) -> Optional[Classification]:
        ...
        if result:
            ...
            logger.info("Direct cache hit found", extra={"cached_name": name, "cached_ethnicity": ethnicity})
    ...
    def _update_classification_cache(self, conn: sqlite3.Connection, record: LLMClassificationRecord):
    ...
    def _find_phonetic_family_match(self, conn: sqlite3.Connection, name: str) -> Optional[Classification]:
        ...
        families = conn.execute('''
            ...
            LIMIT 5
        ''', (soundex_code, metaphone_code)).fetchall()
        ...
        if families:
            ...
            adjusted_confidence = confidence * min(1.0, member_count / 5.0)  # Boost for larger families
            
            if adjusted_confidence >= 0.5:
                logger.info("Phonetic family match found",
                           extra={"matched_name": name,
    ...
    def _find_linguistic_pattern_match(self, conn: sqlite3.Connection, name: str) -> Optional[Classification]:
        ...
        if best_match and best_confidence >= 0.6:
            ...
            logger.info("Linguistic pattern match found",
                       extra={"matched_name": name,
    ...
    def get_learning_statistics(self) -> Dict[str, Any]:
        ...
        with self._db_lock:
            with sqlite3.connect(self.db_path, timeout=30.0) as conn:
                ...
                total_llm = conn.execute('SELECT COUNT(*) FROM llm_classifications').fetchone()[0]
                ...
                # Phonetic families
                phonetic_families = conn.execute('SELECT COUNT(*) FROM phonetic_families').fetchone()[0]
    ...
    def cleanup_old_data(self, days_to_keep: int = 365):
        ...
        with sqlite3.connect(self.db_path, timeout=30.0) as conn:
            ...
            logger.info("Learning database cleanup completed",
