...
try:
    ...
    logging.warning(
        "jellyfish library not available - phonetic matching will be limited"
    )

from .dictionaries import EthnicityType, get_dictionaries
from .exceptions import PhoneticMatchingError
from .models import (
...
class PhoneticClassifier:
    ...
    def _precompute_phonetic_cache(self) -> None:
        ...
        self.phonetic_cache: Dict[
            str, Dict[str, Set[Tuple[str, EthnicityType, float]]]
            ...
            for name, entry in dictionary.items():
                ...
                for algorithm, code in phonetic_codes.items():
                    if code:  # Only store valid codes
                        self.phonetic_cache[algorithm][code].add(
                            (name, ethnicity, entry.confidence)
                        )

        total_cached = sum(
            len(codes)
            for algorithm_cache in self.phonetic_cache.values()
            for codes in algorithm_cache.values()
        )
        logger.info(
            f"Precomputed {total_cached} phonetic mappings across {len(self.algorithm_weights)} algorithms"
        )
    ...
    async def classify_name(self, name: str) -> Optional[Classification]:
        ...
        try:
            ...
            classification = self._analyze_matches(
                name, all_matches, phonetic_codes, start_time
            )
    ...
    def _find_matches_for_algorithm(
        self, algorithm: str, code: str, original_name: str
    ) -> List[PhoneticMatch]:
        ...
        if algorithm == "jaro_winkler":
            ...
            if code in self.phonetic_cache[algorithm]:
                for cached_name, ethnicity, confidence in self.phonetic_cache[
                    algorithm
                ][code]:
                    # Calculate string similarity for additional confidence
                    similarity = SequenceMatcher(
                        None, original_name.lower(), cached_name
                    ).ratio()
        ...
        for ethnicity, dictionary in self.dictionaries.dictionaries.items():
            for cached_name, entry in dictionary.items():
                try:
                    ...
                    logger.debug(
                        f"Jaro-Winkler failed for '{name}' vs '{cached_name}': {e}"
                    )
                    continue
    ...
    def _calculate_consensus_score(
        ...
        ethnicity_counts: Counter[EthnicityType] = Counter()
        for match in matches:
        ...
        agreeing_algorithms = sum(
            1
            for alg_ethnicities in algorithm_votes.values()
        ...
        try:
            ...
            for match in all_matches:
                if (
                    ...
                    and match.matched_name not in seen_names
                ):
    ...
    def get_phonetic_stats(self) -> Dict[str, Any]:
        ...
        for algorithm, cache in self.phonetic_cache.items():
            stats["cached_phonetic_mappings"][algorithm] = len(cache)  # type: ignore
