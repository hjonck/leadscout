...
class CostMonitor:
    ...
    def __init__(
        self, session_limit: float = 10.0, per_classification_limit: float = 0.001
    ):
...
class LLMClassifier:
    ...
    def _get_few_shot_examples(
        self, target_name: str, num_examples: int = 3
    ) -> List[Tuple[str, str]]:
        ...
        for ethnicity, dictionary in self.sa_dictionaries.dictionaries.items():
            ...
            high_conf_names = [
                entry.name for entry in dictionary.values() if entry.confidence >= 0.9
            ]

            # Select diverse examples (mix of surnames and forenames if available)
            if high_conf_names:
                selected = random.sample(
                    high_conf_names, min(num_examples, len(high_conf_names))
                )
...
Consider SA cultural patterns: Nguni, Tamil, Cape Muslim, Afrikaans origins."""
        ...
        if few_shot_examples:
            examples_text = "\nExamples:\n"
            for example_name, ethnicity in few_shot_examples[
                :8
...
Consider SA patterns: Nguni, Tamil, Cape Muslim, Afrikaans origins."""
    ...
    def _parse_single_response(
        self, content: str, name: str
    ) -> Tuple[EthnicityType, float]:
    ...
    def _parse_batch_response(
        self, content: str, names: List[str]
    ) -> List[Tuple[EthnicityType, float]]:
    ...
    async def classify_name(
        ...
        if not self.cost_monitor.should_allow_request():
            logger.warning(
                f"Cost limit reached, skipping LLM classification for '{name}'"
            )
            return None
        ...
        try:
            ...
            if not parsed_response and retry_count < self.max_retries:
                logger.info(
                    f"Retrying classification for '{name}' (attempt {retry_count + 1})"
                )
        ...
        for i in range(0, len(names), batch_size):
            ...
            try:
                ...
                if parsed_response:
                    ...
                    for j, (name, (ethnicity, confidence)) in enumerate(
        ...
        return {
            ...
            "performance": {
                ...
                "avg_response_time_ms": round(
                    ...
                    / max(self.cost_monitor.classification_count, 1),
                    1,
                ),
            },
            "providers": {
    ...
    def get_cost_estimate(self, num_names: int) -> Dict[str, Any]:
        ...
        single_cost = (tokens_per_single / 1000) * self.cost_per_1k_tokens[
            self.primary_model
        ]["input"]
        ...
        batch_total = (batch_tokens / 1000) * self.cost_per_1k_tokens[
            self.primary_model
        ]["input"]
        ...
        return {
            ...
            "batch_requests": {
                ...
                "savings_vs_single": round(
                    (single_total - batch_total) / single_total * 100, 1
                ),
            },
            "research_targets": {
