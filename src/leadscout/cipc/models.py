...
class CIPCCompany(BaseModel):
    ...
    province: Optional[str] = Field(
        None,
        max_length=50,
        description="Inferred province location"
    )
    industry_keywords: List[str] = Field(
        default_factory=list,
        description="Industry classification keywords"
    )
...
class PersonalName(BaseModel):
    ...
    position: int = Field(
        ...,
        ge=0,
        description="Position within company name (0-indexed)"
    )
    ...
    def validate_extraction_method(cls, v: str) -> str:
        ...
        if v not in valid_methods:
            raise ValueError(f"Invalid extraction method. Must be one of: {valid_methods}")
        return v
...
class SearchFilters(BaseModel):
    ...
    company_status: Optional[CompanyStatus] = Field(None, description="Filter by company status")
    active_only: bool = Field(True, description="Include only active companies")
    min_confidence: float = Field(0.0, ge=0, le=1, description="Minimum extraction confidence")
    has_personal_names: Optional[bool] = Field(None, description="Filter by presence of personal names")
    download_batch_from: Optional[date] = Field(None, description="Minimum download batch date")
    download_batch_to: Optional[date] = Field(None, description="Maximum download batch date")
...
class CompanyMatch(BaseModel):
    ...
    similarity_score: float = Field(..., ge=0, le=1, description="Similarity to search query")
    match_type: str = Field(..., description="Type of match (exact, fuzzy, partial)")
    match_fields: List[str] = Field(..., description="Fields that contributed to match")
    ...
    def validate_match_type(cls, v: str) -> str:
        ...
        if v not in valid_types:
            raise ValueError(f"Invalid match type. Must be one of: {valid_types}")
        return v
    ...
    def is_high_quality_match(self, threshold: float = 0.8) -> bool:
        ...
        return (self.similarity_score >= threshold and 
                ...
                self.match_type in {'exact', 'fuzzy'})
...
class CompanySearchRequest(BaseModel):
    ...
    query: str = Field(..., min_length=2, max_length=255, description="Company name search query")
    ...
    fuzzy_threshold: float = Field(0.6, ge=0, le=1, description="Minimum fuzzy match score")
    max_results: int = Field(20, ge=1, le=100, description="Maximum number of results")
    include_inactive: bool = Field(False, description="Include inactive companies")
    ...
    def validate_sort_by(cls, v: str) -> str:
        ...
        if v not in valid_sorts:
            raise ValueError(f"Invalid sort criteria. Must be one of: {valid_sorts}")
        return v
...
class CompanySearchResponse(BaseModel):
    ...
    has_more_results: bool = Field(False, description="Whether more results are available")
...
class CIPCDownloadBatch(BaseModel):
    ...
    download_date: datetime = Field(default_factory=datetime.utcnow, description="When download was performed")
    files_downloaded: int = Field(0, ge=0, le=26, description="Number of CSV files downloaded")
    total_companies: int = Field(0, ge=0, description="Total companies processed")
    total_names_extracted: int = Field(0, ge=0, description="Total personal names extracted")
    processing_time_minutes: int = Field(0, ge=0, description="Total processing time")
...
class CIPCProcessingStats(BaseModel):
    ...
    total_companies_processed: int = Field(0, ge=0, description="Total companies processed")
    total_names_extracted: int = Field(0, ge=0, description="Total personal names extracted")
    successful_downloads: int = Field(0, ge=0, description="Successful CSV downloads")
    failed_downloads: int = Field(0, ge=0, description="Failed CSV downloads")
    average_processing_time_ms: float = Field(0.0, ge=0, description="Average processing time per company")
    extraction_accuracy_rate: float = Field(0.0, ge=0, le=1, description="Name extraction accuracy rate")
    cache_hit_rate: float = Field(0.0, ge=0, le=1, description="Cache hit rate for searches")
    last_batch_date: Optional[date] = Field(None, description="Date of last processed batch")
    updated_at: datetime = Field(default_factory=datetime.utcnow, description="Statistics update timestamp")
