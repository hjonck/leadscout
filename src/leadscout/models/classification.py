...
class Classification(BaseModel):
    ...
    name: str = Field(
        ...
        min_length=1,
        max_length=255,
        description="Original name classified",
    )
    ethnicity: EthnicityType = Field(
        ..., description="Predicted ethnicity category"
    )
    confidence: float = Field(
        ..., ge=0, le=1, description="Classification confidence score"
    )
    method: str = Field(..., description="Classification method used")
    phonetic_codes: Optional[Dict[str, str]] = Field(
        None, description="Phonetic algorithm results"
    )
    ...
    def validate_method(cls, v: str) -> str:
        ...
        if v not in valid_methods:
            raise ValueError(
                f"Invalid classification method. Must be one of: {valid_methods}"
            )
        return v
...
class PhoneticResult(BaseModel):
    ...
    weight: float = Field(
        1.0, ge=0, le=1, description="Algorithm weight for scoring"
    )
    ...
    def validate_algorithm(cls, v: str) -> str:
        ...
        if v not in valid_algorithms:
            raise ValueError(
                f"Invalid algorithm. Must be one of: {valid_algorithms}"
            )
        return v
...
class ClassificationMatch(BaseModel):
    ...
    matched_classification: Classification = Field(
        ..., description="Classification of matched name"
    )
    ...
    def validate_match_method(cls, v: str) -> str:
        ...
        if v not in valid_methods:
            raise ValueError(
                f"Invalid match method. Must be one of: {valid_methods}"
            )
        return v
    ...
    def is_reliable_match(self, threshold: float = 0.8) -> bool:
        ...
        return (
            ...
            and self.matched_classification.is_high_confidence()
        )
...
class ClassificationStats(BaseModel):
    ...
    total_classifications: int = Field(
        0, ge=0, description="Total classifications performed"
    )
    ...
    high_confidence_results: int = Field(
        0, ge=0, description="High-confidence results"
    )
    processing_time_ms: float = Field(
        0.0, ge=0, description="Average processing time"
    )
    accuracy_rate: Optional[float] = Field(
        None, ge=0, le=1, description="Estimated accuracy rate"
    )
    ...
    def get_high_confidence_rate(self) -> float:
        ...
        if self.total_classifications == 0:
            return 0.0
        return (
            self.high_confidence_results / self.total_classifications
        ) * 100
