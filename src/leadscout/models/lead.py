...
class Lead(BaseModel):
    ...
    entity_name: str = Field(
        ..., min_length=2, max_length=255, description="Legal business name"
    )
    trading_as_name: str = Field(
        ..., min_length=2, max_length=255, description="Trading/brand name"
    )
    keyword: str = Field(
        ..., min_length=2, max_length=100, description="Industry/business type"
    )
    ...
    registered_address: str = Field(
        ..., min_length=10, max_length=500, description="Full business address"
    )
    registered_address_city: str = Field(
        ..., min_length=2, max_length=100, description="City name"
    )
    registered_address_province: str = Field(
        ..., min_length=2, max_length=100, description="Province/state"
    )
    director_name: str = Field(
        ..., min_length=2, max_length=255, description="Director/owner name"
    )
    director_cell: str = Field(
        ..., min_length=10, max_length=20, description="Director mobile number"
    )
    ...
    def get_hash(self) -> str:
        ...
        key_data = (
            f"{self.entity_name}|{self.trading_as_name}|{self.email_address}"
        )
...
class EnrichedLead(BaseModel):
    ...
    classification: Optional[Classification] = Field(
        None, description="Name ethnicity classification"
    )
    contact_validation: Optional[ContactValidation] = Field(
        None, description="Contact validation results"
    )
    website_url: Optional[str] = Field(
        None, description="Discovered company website"
    )
    linkedin_profile: Optional[str] = Field(
        None, description="Director LinkedIn profile URL"
    )

    # Data availability flags
    website_found: bool = Field(
        False, description="Website discovery success flag"
    )
    linkedin_found: bool = Field(
        False, description="LinkedIn profile found flag"
    )

    # Scoring metrics
    data_richness_score: float = Field(
        0.0, ge=0, le=100, description="Data availability score"
    )
    contact_quality_score: float = Field(
        0.0, ge=0, le=100, description="Contact quality score"
    )
    priority_score: float = Field(
        0.0, ge=0, le=100, description="Final prioritization score"
    )
    confidence_level: float = Field(
        0.0, ge=0, le=1, description="Overall confidence level"
    )
    ...
    def get_enrichment_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "classification": self.classification.ethnicity
