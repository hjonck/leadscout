...
class ContactValidation(BaseModel):
    ...
    email_deliverable: Optional[bool] = Field(None, description="Email deliverability status")
    
    # Phone validation
    phone_valid: bool = Field(False, description="Phone format is valid")
    phone_reachable: Optional[bool] = Field(None, description="Phone reachability status")
    
    # Mobile validation
    mobile_valid: bool = Field(False, description="Mobile format is valid")
    mobile_reachable: Optional[bool] = Field(None, description="Mobile reachability status")
    
    # Overall metrics
    overall_score: float = Field(0.0, ge=0, le=100, description="Overall contact quality score")
    validation_method: str = Field("format", description="Validation method used")
    ...
    def validate_method(cls, v: str) -> str:
        ...
        if v not in valid_methods:
            raise ValueError(f"Invalid validation method. Must be one of: {valid_methods}")
        return v
    ...
    def get_validation_summary(self) -> dict[str, any]:
        ...
        return {
            ...
            'has_reachable_phone': self.phone_reachable is True or self.mobile_reachable is True,
